...
significant_snps_summary <- results$summary %>%
                      ...
                      separate(ALT_ALLELE, into = paste0("REF_ALLELE_", ancestries_susiex_order), sep = ",") %>%
...
write.csv(significant_snps_summary,'manuscript/tables/susiex_significant_summary.csv', row.names = FALSE, quote = FALSE)


items_to_keep <- sapply(results$cs, function(results_table){
  sig <- results_table %>%
    filter(CHR %in% significant_snps$CHR & BP_START %in% significant_snps$BP_START)
  nrow(sig) > 0
})
...
write.csv(significant_snps_cs, 'manuscript/tables/susiex_significant_cs.csv', row.names = FALSE, quote = FALSE)
